...
export function ticketDetailsReducer(state: Array<TicketDetail> = [], action: TicketDetailActions.Actions): Array<TicketDetail>
{
    switch(action.type)
    {
        ...
        case SharedActions.UPDATE_TICKET_STATUS:
            return state.map(ticket => ticket.id === action.payload.ticketId ? Object.assign({}, ticket, { status: action.payload.toStatus, events: [...ticket.events, action.payload]}) : ticket); 
        ...
        case SharedActions.ADD_TICKET_COMMENT:
            return state.map(ticket => ticket.id === action.payload.ticketId ? Object.assign({}, ticket, { events: [...ticket.events, action.payload] }) : ticket);  
        default:
            return state; 
    }
}